...
function print(data) {
    ...
    if (data.TYPE === RECEIPT_TYPE) {
        ...
        if (data.ORDERS.draftBeer) {
            let printer = printers.find(function (p) {
                // console.log(p.DETAILS.sectionRef)
                return p.DETAILS.roles.ORDER === true &&
                    p.DETAILS.sectionRef === 'fıçıbira'
            })

            console.log(printer.DETAILS)

            if (!printer) {
                console.log("no printer available for draftBeer")
                return
            }

            printer.println(data.ORDERS.draftBeer);
            printer.cut();
            ...
            try {
                ...
                printer.beep(); // Sound internal beeper/buzzer (if available)
                let execute = printer.execute();
                console.log("Print done draftBeer with new!");
                // await printer.printImage(".touch.png"); // Print PNG image
            } catch (error) {
                console.log("Print failed with new:", error);
            }
